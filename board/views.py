...
from .models import Post, Comment, Message, alarm_push, Review, ChatMessage
from .forms import PostForm, CommentForm, MessageForm, ChatMessageForm
...
def warning_comment(request, pk):
    comment = get_object_or_404(Comment, pk=pk)
    com_reported_user = comment.author

    com_reported_user.warning += 1
    com_reported_user.save()

    if com_reported_user.warning >= 5:
        com_reported_user.is_active = False
        com_reported_user.save()
        return redirect('board:community')
...
class MyProfileView(View):
    def get(self, request):
        user = request.user
        profile = get_object_or_404(get_user_model(), username=user.username)
...
def coin_view(request):
    return render(request, 'board/coin.html')
...
#이용내역
def use_content(request):
    user = request.user
    comments = Comment.objects.filter(author=user)
    posts = Post.objects.filter(author=user)
    profile = get_object_or_404(get_user_model(), id=user.id)
    ...
    return render(request, 'board/receipe.html', context)
...
def chat_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('board:chat_view', pk=pk)  # 현재 페이지로 리다이렉트하여 새로운 댓글을 표시
    else:
        form = ChatMessageForm()

    chat_messages = ChatMessage.objects.all().order_by('-timestamp')[:10]  # 최근 10개의 메시지 조회
    return render(request, 'board/chat.html', {'form': form, 'chat_messages': chat_messages, 'profile': profile})
